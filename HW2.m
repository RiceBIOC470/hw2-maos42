...
%average of the columns(?)) and has a NaN in the column that contained the NaN whereas 
%zz is a single number (the average of all non-NaN entries in xx). Explain
%this behavior. 

%Miguel Angel: The solution is within your function meanonan. After you put
%the NaN in xx, yy cannot do the average because of it, that is why the
%second column will always get a NaN. However, after you use your function,
%what is doing is checking first if there is a NaN, +Inf or -Inf, and if
%there is one, to substitute it with an empty value. That way it
%automatically changes xx and all the values are numbers now. Thus the
%average of all the values is possible.
...
%Miguel Angel: this is the meannonan function already modified:
meannonan(xx)
...
%Miguel Angel: the first input is the length of the sequence, the second
%the minimum size of the ORFs.

probabilityORF(500,50)
...
%Miguel Angel: you only have to input the length of the desired ORFs. 
plotProbabilityORF(50)
